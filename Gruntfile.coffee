...
module.exports = (grunt) ->
    ...
    grunt.registerTask 'gen-examples', 'Generate an example for each theme', ->
        ...
        render = (name, done) ->
            ...
            aglio.renderFile 'xtron.apib', "examples/#{name}.html", themeVariables: name, (err) ->
                if err then return done(err)
                console.log "Generating examples/#{name}-triple.html"
                aglio.renderFile 'xtron.apib', "examples/#{name}-triple.html", themeVariables: name, themeTemplate: 'triple', (err) ->
                    done(err)
