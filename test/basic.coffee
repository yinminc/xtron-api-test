...
blueprint = fs.readFileSync path.join(root, 'xtron.apib'), 'utf-8'
...
describe 'API Blueprint Renderer', ->
    ...
    it 'Should render to stdout', (done) ->
        ...
        aglio.renderFile path.join(root, 'xtron.apib'), '-', 'default', (err) ->
            if err
                console.log.restore()
                return done(err)
    ...
    it 'Should compile to stdout', (done) ->
        ...
        aglio.compileFile path.join(root, 'xtron.apib'), '-', (err) ->
            if err then return done(err)
    ...
    it 'Should error on non-file failure', (done) ->
        ...
        aglio.renderFile path.join(root, 'xtron.apib'), 'bar', 'default', (err, html) ->
            assert err
...
describe 'Executable', ->
    ...
    it 'Should render a file', (done) ->
        ...
        bin.run i: path.join(root, 'xtron.apib'), o: '-', ->
    ...
    it 'Should compile a file', (done) ->
        ...
        bin.run c: 1, i: path.join(root, 'xtron.apib'), o: '-', ->
    ...
    it 'Should start a live preview server', (done) ->
            ...
            listen: (port, host, cb) ->
                ...
                res =
                    writeHead: (status, headers) -> false
                        ...
                        aglio.render.restore()
                        cb()
                        file = fs.readFileSync 'xtron.apib', 'utf8'
                        setTimeout ->
                            fs.writeFileSync 'xtron.apib', file, 'utf8'
                            setTimeout ->
                                console.log.restore()
                                done()
                            , 500
        ...
        bin.run s: true, (err) ->
            ...
            bin.run i: path.join(root, 'xtron.apib'), s: true, p: 3000, h: 'localhost', (err) ->
    ...
    it 'Should support custom Jade template shortcut', (done) ->
        ...
        bin.run i: path.join(root, 'xtron.apib'), t: 'test.jade', o: '-', (err) ->
            console.log.restore()
            done(err)
